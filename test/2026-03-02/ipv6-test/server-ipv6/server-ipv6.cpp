...
/**
 * Server Ipv6 (Sunucu Ipv6)
 * 
 * Sunucu ipv4 testini yapmıştık ve başarı ile sonuçlanmıştı.
 ...
 * 
 * Derleme:
 *  Bsd     :: g++ -I../../../../include -std=c++17 -Wall -Werror -Wextra server-ipv6.cpp -pthread -o bsd/server-ipv6.bsd
 *  Linux   :: g++ -I../../../../include -std=c++17 -Wall -Werror -Wextra server-ipv6.cpp -o linux/server-ipv6.linux
 *  Windows :: g++ -I../../../../include -std=c++17 -Wall -Werror -Wextra server-ipv6.cpp -o windows/server-ipv6.exe -lws2_32
 * 
 * Çalıştırma:
 *  Bsd     :: ./bsd/server-ipv6.bsd
 *  Linux   :: ./linux/server-ipv6.linux
 *  Windows :: ./windows/server-ipv6.exe
 */
...
static constexpr Vch<16> ss_verhash("20260302|test|server|ipv6", 20260302);

static const std::string ss_osname = utf::to_lower(current_os_name());
static const std::string ss_logname = "server-ipv6-" + ss_osname;
...
int main(void)
{
    ...
    Crypto<CipherEnc> vv_cipher("ipv6-server-cipher", utf::to_utf8(U"key-cipher-server@20260302"));
    Socket<CipherEnc> vv_server(vv_cipher, "logs/socket-" + ss_logname, "netsocket-server", "pwd@server.ipv6!test", 2026, ipv_t::ipv6, _FLAG_SOCKET_LOGGER | _FLAG_SOCKET_LOG_TITLE);
