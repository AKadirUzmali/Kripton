 ...
 * 
 * Çalıştırma:
 *  Bsd     :: ./bsd/client-test.bsd <ip version> <ip address>
 *  Linux   :: ./linux/client-test.linux <ip version> <ip address>
 *  Windows :: ./windows/client-test.exe <ip version> <ip address>
 * 
 * Örnek:
 *  Bsd     :: ./bsd/client-test.bsd v6 ::1
 *  Linux   :: ./linux/client-test.linux v4 127.0.0.1
 *  Windows :: ./windows/client-test.exe v4 192.168.1.108
 */
...
int main(int argc, char* argv[])
{
    ...
    CipherEnc vv_cipher("loop-client-cipher", utf::to_utf8(U"key-cipher-server@20260312"));
    ...
    // CLIENT
    tm_status = client_session(vv_client, vv_ip);
...
{
    if( !Socket::is_valid_ipv4(ar_ip) && !Socket::is_valid_ipv6(ar_ip) )
        return Status::err(domain_t::client, 20000);

    if( !Socket::is_valid_socket(ar_sock.get_socket()) )
    ...
    Status tm_handshake = Socket::handshake_send_verify(ar_sock.get_socket());
    ...
    tm_handshake = Socket::handshake_recv_verify(ar_sock.get_socket());
    if( !tm_handshake.is_ok() )
    {
        if( ar_sock.get_flag().has(_FLAG_SOCKET_LOGGER) )
    ...
    for(int tm_count = 0; tm_count < vv_total_time; ++tm_count)
    {
        ...
        {
            if( ar_sock.get_flag().has(_FLAG_SOCKET_LOGGER) )
                ar_sock.get_logger().write(level_t::Err, "Data couldn't send to " + Socket::get_ip(ar_sock.get_socket()), GET_SOURCE);
        ...
        {
            if( ar_sock.get_flag().has(_FLAG_SOCKET_LOGGER) )
                ar_sock.get_logger().write(level_t::Err, "Data couldn't receive from " + Socket::get_ip(ar_sock.get_socket()), GET_SOURCE);
